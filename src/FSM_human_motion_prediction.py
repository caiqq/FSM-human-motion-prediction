...
tf.app.flags.DEFINE_integer("iterations", int(6*1e3), "Iterations to train for.")
# tf.app.flags.DEFINE_integer("iterations", 2, "Iterations to train for.")
# Architecture
...
tf.app.flags.DEFINE_integer("save_log_every", 500, "How often to compute error on the test set.")
...
def normalization_dict_data(dict_data):
    scaler1 = preprocessing.MinMaxScaler()
    for dict_key, dict_value in dict_data.items():
        dict_data[dict_key] = scaler1.fit_transform(dict_value)
    return dict_data


def spike_encode(data):
    # data = np.transpose(data, (0, 2, 1))  # (batch, feature, time)

    # sca = data.shape[-1] * 10  #
    sca = 100  # 500
    time_length = 110

    data = (data * sca).astype(int)  # (batch, feature, 49)

    encode = np.zeros((data.shape[0], data.shape[1], data.shape[2], time_length))

    for k in range(data.shape[0]):  # batch_size

        for i in range(data.shape[1]):  # feature

            for j in range(data.shape[2]):

                data_ki = data[k, i, j]
                # indx = data_ki[data_ki > 0]
                if data_ki >0:
                    encode[k, i, j, data_ki] = 1

    return encode


def train(action, train_dir, summaries_dir):
    ...
    print(summaries_dir + 'test.txt')
    test_file = open(summaries_dir + 'test.txt', 'w')

    train_set, test_set, data_mean, data_std, dim_to_ignore, dim_to_use = read_all_data(
        actions, FLAGS.seq_length_in, FLAGS.seq_length_out, FLAGS.data_dir, not FLAGS.omit_one_hot)

    # normalization_dict_data(train_set)
    # normalization_dict_data(test_set)
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options, device_count=device_count)) as sess:
        ...
        model = create_model(sess, actions, train_dir, summaries_dir)
        ...
        for _ in xrange(FLAGS.iterations):
            ...
            # encoder_inputs = spike_encode(encoder_inputs_x)
            # decoder_inputs = spike_encode(decoder_inputs_x)
            ...
            if current_step % FLAGS.test_every == 0:
                ...
                # encoder_inputs = spike_encode(encoder_inputs)
                # decoder_inputs = spike_encode(decoder_inputs)
                ...
                for action in actions:
                    ...
                    for i in np.arange(N_SEQUENCE_TEST):
                        ...
                        a1 = srnn_gts_euler[action][i][:, idx_to_use]
                        a2 = eulerchannels_pred[:, idx_to_use]
                        euc_error = np.power(
                    ...
                    for ms in [1, 3, 7, 9, 13, 24]:
                        if FLAGS.seq_length_out >= ms + 1:
                            ...
                            print(" {0:.3f} |".format(mean_mean_errors[ms]), end="", file=test_file)
                        else:
                            print("   n/a |", end="")
                            print("   n/a |", end="", file=test_file)
                ...
                # Save the model
                # if current_step % FLAGS.save_log_every == 0:
                #     print("{0: <16} |".format(action), end="")
                #     for ms in [1, 3, 7, 9, 13, 24]:
                #         if FLAGS.seq_length_out >= ms + 1:
                #             print(" {0:.3f} |".format(mean_mean_errors[ms]), end="", file=test_file)
...
def sample(train_dir, summaries_dir):
    ...
    with tf.Session(config=tf.ConfigProto(device_count=device_count)) as sess:
        ...
        print("Creating %d layers of %d units." % (FLAGS.num_layers, FLAGS.size))
        sampling = True
        model = create_model(sess, actions, train_dir, summaries_dir, sampling)
        print("Model created")
        ...
        # train_set = normalization_dict_data(train_set)
...
def main(_):
    ...
    if FLAGS.sample:
        actions = ["walking", "eating", "smoking", "discussion", "directions",
                   ...
                   "walkingtogether"]
        for action in actions:
            train_dir = os.path.normpath(os.path.join(FLAGS.train_dir, action,
                                                      'multitask_transfer',
            ...
            sample(action, train_dir, summaries_dir)
        ...
        for action in actions:
            ...
            summaries_dir = os.path.normpath(os.path.join(train_dir, "log"))  # Directory for TB summaries
            train(action, train_dir, summaries_dir)
